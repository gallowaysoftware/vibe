...
Cells appearing (llamaloft built, sparks commissioned) or disappearing (localmodel
gaming) changes the *catalog*, never the *address*. When localmodel relocates
...
- localmodel gaming: either leave it (VRAM-exceeded surfaces as a clean
...
(`qwen3.6-27b-mtp-q6_k` on localmodel; later `qwen3-coder-next` on spark-2,
...
on localmodel + text on sparks + TTS on llamaloft) are just per-stage base URLs on
...
5. **A8b cleanup:** once llamaloft hosts bge-embed, delete vibe's proxy.go +
