...
Every mechanically verifiable gate is green. The live gates ran against
the local multi-cell harness on 2026-08-05 (#34) and again under
[C17](c17-gate-closure.md): **live halves of 2, 3, 5, 6 and 8 PASS**;
1 and 4 stay PARTIAL and each names the one physical fact it lacks —
24 h of wall clock, and an mlx cell. See
...
| gate | result |
|---|---|
| 1. Store gate (24 h soak) | **PARTIAL** — the §0 claim ran compressed on the local multi-cell harness 2026-08-05: 1200 embeddings driven at one cell in 23 s took `/api/metrics/activity` `total` from 21 to 1221 and sqlite `min(id),max(id)` to `1 / 1221` — nothing pruned, so `store: {path:}` genuinely retires the 1000-row in-memory ring. A `swapctl stop`/`start` kept `total` and both ids. **The 24 h duration itself was not spanned.** Re-assessed by C17 (2026-08-05): the one thing missing is 24 h of wall clock — no hardware, no second box, no second architecture — and the harness can run it unattended. |
| 2. No-double-count | unit half **PASS** (`TestUsageFold_FrontCellIsSkippedStructurallyByName`); live half **PASS (2026-08-05, C17, `scripts/fleetlab/gate-c7a-partials.sh`)** — the earlier run proved the skip by OUTCOME only, because the harness front had no collector at all. This one commissions one: a slim `vibe fleet announce --cell front` against the front's own llama-swap, then 10 requests client → front → alpha. The front's collector reported cumulative totals **on the wire** (`req 10, in_fresh 48, in_cached 342, out 40, poke_req 3, err_req 1`, cursor `last_row_id 14`), `/api/fleet/usage` carried **0** front buckets, `usage.jsonl` carried **0** of 35 lines with `cell=front`, and alpha's row moved to `req 19` — the requests were billed exactly once, to the cell that served them. Non-zero front rows reached the fold and were refused by name. |
...
| 4. Unmeasured | unit half **PASS** (`TestClassify_ZeroTokenTwoHundredIsUnmeasuredNotZero`, `TestClassify_NonTwoHundredIsErrorAndContributesNothing`); live half **PARTIAL** — the error branch ran: two over-context chats returned 400, llama-swap wrote rows with status 400 and all token fields 0, and the ledger moved `err_req 10 → 12` with every token sum unchanged. The **cancelled-stream** branch could not be produced: SIGKILLing a client 3 s into a stream still left this llama-server build reporting `timings`, so the row metered normally (`status 200, output_tokens 32`) and `unmeasured_req` stayed 0. That is an upstream-build property, not a vibe defect — the branch needs a server that omits `timings` on abort. C17 (2026-08-05) names the missing physical fact precisely: an **mlx** cell, i.e. a second machine of a different architecture; no llama.cpp build on this box produces the row. |
| 5. Self-traffic | unit half **PASS** (`TestClassify_OneTokenChatRowIsAPokeExcludedFromBillableSums`); live half **PASS (2026-08-05, C17, `scripts/fleetlab/gate-c7a-partials.sh`)** — the earlier run left two cases unexercised and both now ran. *A poke that is NOT one token:* `max_tokens=1` (completion 1) moved `poke_req` 4→5 with `req` unchanged at 19 and `out` unchanged at 496; `max_tokens=6` (completion 6) moved `req` 19→20, `out` 496→502, `in_fresh` +9 and left `poke_req` at 5 — the `output_tokens <= 1` boundary, both sides of it, against the two llama-swap rows behind the delta (id 26 out=1, id 27 out=6). *Probe traffic as a second producer:* 6 chat probe rows on alpha (101.7 tokens each) and 13 embed probe rows on charlie (768.0 each) are classified `req`, never `poke_req` — exactly as C8's "Known and accepted" says, and see [C17 finding 2](c17-gate-closure.md#finding-2-a-documented-number-that-is-wrong-the-probe-traffic-envelope-is-a-chat-figure) for the envelope that costs. Earlier evidence retained: the warm target's real 1-token completions reconcile exactly against llama-swap's own totals (66 input / 24 cache / 3 output, entirely 3 pokes). |
