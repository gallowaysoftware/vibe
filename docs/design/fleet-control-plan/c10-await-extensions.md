...
refuses to create — and **13d PASSED on the same harness under
[C17](c17-gate-closure.md)**, which also found that the gate's own flag
list cannot be typed (see its row). See
...
| 13d. two shells, B waits for A's lease | **PASS (2026-08-05, C17, `scripts/fleetlab/gate-c10-13d.sh`)** — two genuinely separate processes against a real cell. A ran `vibe cell await bravo --model lab-embed-b --ready --idle 20s --unleased --lease batchA --lease-ttl 90s` and unblocked at **exactly 20 s** (`bravo is up: lab-embed-b ready; idle 20s (>= 20s); no other leases` / `lease held: batchA on bravo/lab-embed-b for 1m30s`). B, started while that lease was live with the same flags under holder `batchB`, printed `await bravo: up; lab-embed-b ready; idle 20s (>= 20s); leased by batchA` and stayed blocked; it unblocked at **155 s**, one 5 s poll after A's lease expired, and took its own. Mid-wait, A's own re-run returned in **0 s** ignoring its OWN holder (`no other leases`) and re-claimed — which is exactly what moved B's unblock from 90 s to 155 s, the lease arithmetic visible end to end. **A correction to the gate text:** it says "`--idle --unleased`", and that cannot be run — `--lease` requires `--model`, and `--model` requires a condition (`vibe: --model needs a condition: use --model <id> --ready`), both refused before the wait by `validateAwaitFlags`. The primitive is `--model X --ready --idle D --unleased --lease H`; the gate should be restated in that form. |
...
| 13. Live gates (a–d) | ran 2026-08-05 on the local multi-cell harness: **13b PASS**, **13a PARTIAL** (semantics yes, cold-start magnitude no), **13c VOID** (describes a state fleetd refuses to create), **13d PASS** (C17, same day, `scripts/fleetlab/gate-c10-13d.sh`). Detail in the [gate table](#gates-re-run-after-the-review-commit) above |
