...
12. **Live gates.** (Written as "need real hardware"; the 2026-08-05
    ...
    c. ~~The announce-only case: a cell fleetd holds no `url` for prints
       the no-evidence line and never unblocks on `--idle`.~~ **Void:**
       fleetd 400s an announce from a cell it holds no entry for, so
       this state cannot exist. Restated as the stream-down case: with
       fleetd's event stream to a cell dropped, `--idle` prints the
       no-evidence line and never unblocks. That version passes.
