...
| 9 live plausibility | **STILL NOT RUN** (re-assessed 2026-08-05). The 2026-08-05 local multi-cell harness run ([`scripts/fleetlab`](../../../scripts/fleetlab/README.md)) moved most of the plan's other live gates, but not this one, and the reason is specific rather than logistical: this gate is a *judgement about a number*. It needs a week of real traffic, priced against a real twin, on cells whose watts and capital cost are measured rather than declared. The harness serves CPU bge embeddings — there is no meaningful frontier twin for them, and a synthetic `watts_idle` in a lab `hosts.yaml` prices a fiction. The route was exercised (it answers, and C12's sweep confirms it is token-only), but nobody has looked at a savings figure and asked whether it is believable. Re-assessed again by [C17](c17-gate-closure.md) 2026-08-05, which names the three physical facts precisely: a **week of real traffic**, cells whose watts are **measured** rather than declared, and a **real open-weight twin** for what they serve. The harness has none of the three — CPU bge embeddings have no meaningful twin, and a synthetic `watts_idle` in a lab `hosts.yaml` prices a fiction. |
